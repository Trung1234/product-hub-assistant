 ...
 * <follow_up_questions>...</follow_up_questions> or ```followup or ```suggestions or ### 💡 Câu Hỏi Gợi Ý
 */
function extractTaggedFollowUpQuestions(content: string): string[] {
  if (!content) return [];
...
export const ChatMessage = React.memo<ChatMessageProps>(
  ({
    ...
    // Extract dynamic follow-up questions generated by the LLM
    const followUpQuestions = useMemo(() => {
      if (isUser || !hasContent || !isLastMessage) return [];
      return extractTaggedFollowUpQuestions(rawMessageContent);
          ...
          {hasContent && (
            <div className="relative flex flex-col w-full mt-2">
              <div className="overflow-hidden break-words text-sm font-normal leading-relaxed text-white w-full">
                ...
                {isLastMessage && followUpQuestions.length > 0 && (
                  <SuggestedQuestionsRenderer questions={followUpQuestions} />
                )}
              </div>
            </div>
          )}
